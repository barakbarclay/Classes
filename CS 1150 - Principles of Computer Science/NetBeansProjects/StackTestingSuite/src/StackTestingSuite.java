public class StackTestingSuite
{
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            StackTester<Integer> tester = new StackTester<>(
                    new GharanaFrontStack<Integer>(
                    new GharanaLinkedList<Integer>()), new UserInput<>(0));
            ...
            while(!done)
            {
                try
                {
                    ...
                    {
                        case 1:
                            switch(tester.getUserInput().getType())
                            {
                                case "Integer": 
                                    Integer intValue =
                                            (Integer)
                                            tester.getUserInput().getValue();
                                    tester.getStack().push(intValue);
                                    break;
                                case "Double":
                                    Double doubleValue = 
                                            (Double)
                                            tester.getUserInput().getValue();
                                    tester.getStack().push(doubleValue);
                                    break;
                                default: 
                                    String stringValue = 
                                            (String)
