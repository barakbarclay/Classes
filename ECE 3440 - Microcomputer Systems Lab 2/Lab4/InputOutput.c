...
int main(void)
{
  ...
  unsigned char clearDisp = 0;
	unsigned int countDirection = 0;
	unsigned char switchAddress = I2C_8574 | I2C_0;
	
	InitI2C();
  InitLCD();
  sprintf(str, "Hello %4d", var);
  OutputString(str);
  OutLCD(0xC0, 0);
  sprintf(str, "Hello %4x", var);
  OutputString(str);
	ConfigureInterrupts();
  EnableInterrupts();
	P4IFG &= ~BIT6;
	...
	while(1){
		if (getSwitchValue == 1){
			...
			mode = mode&0x0F;
			mode = mode % 4;
      clearDisp = InI2C(switchAddress);
			clearDisp = InI2C(switchAddress);
			clearDisp = InI2C(switchAddress);
			clearDisp = clearDisp >> 3;
			clearDisp = clearDisp&0x0F;
			clearDisp = clearDisp % 2;
      var = InI2C(switchAddress);
			var = InI2C(switchAddress);
			var = InI2C(switchAddress);
			var = var >> 4;
			var = var&0x0F;
